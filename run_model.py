...
from src.sales_insights import (
    calculate_sales_insights,
    get_available_date_range,
    insights_to_dataframe,
    print_sales_insights,
    prompt_for_period,
    prompt_save_insights,
)
...
def train_pipeline() -> None:
    ...
    # Prompt user for the month to analyze in sales insights.
    daily_series = daily_input.set_index("date")["daily_total_sales"]
    period_start, period_end = prompt_for_period(daily_series)
    ...
    if prompt_save_insights():
        ...
        try:
            ...
            print(
                f"\nError: Cannot save to {insights_path} (file is locked). "
                f"Please close the file if it's open in another application and try again.",
                flush=True,
            )
        except Exception as e:
            print(f"\nError saving insights: {e}", flush=True)
